...
def disasm(code):
    ...
    while i<len(code):
        ...
        if 0x60<=op<=0x7f:
            n=op-0x5f
            data=code[i+1:i+1+n]
            out.append((pc,f'PUSH{n}','0x'+data.hex()))
            i+=1+n
        elif 0x80<=op<=0x8f:
            out.append((pc,f'DUP{op-0x7f}',''));i+=1
        elif 0x90<=op<=0x9f:
            out.append((pc,f'SWAP{op-0x8f}',''));i+=1
        else:
            out.append((pc,OPCODES.get(op,f'UNK_{op:02x}'),''));i+=1
    return out
h=open(sys.argv[1]).read().strip()
if h.startswith('0x'):h=h[2:]
code=bytes.fromhex(h)
for pc,mn,arg in disasm(code):
    print(f'{pc:04x}  {mn:12s} {arg}')
